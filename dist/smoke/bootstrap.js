...
var enforceThresholds = (__ENV.ENFORCE_THRESHOLDS ?? "false").toLowerCase() === "true";
var statusTrend = new Trend("status_codes", true);
var statusCounts = {};
var statusOrder = [];
var authErrorCounter = new Counter("auth_errors");
var clientErrorCounter = new Counter("client_errors");
var serverErrorCounter = new Counter("server_errors");
...
function runScenario() {
  ...
  try {
    ...
    if (existing.length < 3) {
      existing.push({
        status: 0,
        message: error?.message ?? "request threw",
        url: targetUrl
      });
    }
  ...
  if (response.status === 0 || response.status >= 400) {
    const bucket = response.status === 401 || response.status === 403 ? "auth" : response.status >= 500 || response.status === 0 ? "server" : "client";
...
function handleSummary(data) {
  if (!enforceThresholds) {
    ...
    const p952 = data.metrics?.http_req_duration?.["p(95)"];
    ...
    if (typeof p952 === "number" && p952 > 2200) {
      console.warn(`http_req_duration threshold exceeded (p95=${p952}ms). Result will still be saved.`);
    }
  }
  const metrics = data.metrics ?? {};
  const fmtPct = (value) => typeof value === "number" ? `${(value * 100).toFixed(2)}%` : "n/a";
  const fmtMs = (value) => typeof value === "number" ? `${value.toFixed(2)} ms` : "n/a";
  const fmtCount = (value) => typeof value === "number" ? value.toLocaleString() : "n/a";
  const fmtRate = (value) => typeof value === "number" ? `${value.toFixed(2)}/s` : "n/a";
  ...
  const successRate = typeof rateFailed === "number" ? Math.max(0, 1 - rateFailed) : void 0;
  ...
  const color = (ok) => ok ? "\x1B[32m" : "\x1B[31m";
  const reset = "\x1B[0m";
  const errorsSection = [
    ...
    authErrors.length ? `- Auth errors: ${authErrors.length} (example status=${authErrors[0].status} url=${authErrors[0].url} msg=${authErrors[0].message})` : "- Auth errors: none",
    clientErrors.length ? `- Client errors: ${clientErrors.length} (example status=${clientErrors[0].status} url=${clientErrors[0].url} msg=${clientErrors[0].message})` : "- Client errors: none",
    serverErrors.length ? `- Server errors: ${serverErrors.length} (example status=${serverErrors[0].status} url=${serverErrors[0].url} msg=${serverErrors[0].message})` : "- Server errors: none"
  ];
  ...
  const statusSection = [
    ...
    `- top codes: ${statusOrder.length ? statusOrder.slice(0, 5).map((code) => `${code} (${statusCounts[code] ?? 0})`).join(", ") : "none captured"}`,
  ...
  const sections = [
    "=== k6 Summary ===",
    `Scenario: ${data.state?.testRunDurationMs ? `${(data.state.testRunDurationMs / 1e3).toFixed(1)}s` : "n/a"}`,
    "",
    ...errorsSection,
    "",
    ...latencySection,
